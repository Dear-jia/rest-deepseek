# 樱梦女仆咖啡厅 · 二次元女仆主题官网 + 后台管理系统

一个二次元女仆主题咖啡厅的完整网站：前台官网（纯静态，可发布到 GitHub Pages）+ 后台管理系统（Spring Boot 3 + Java 17）。女仆与主厨展示、蛋包饭与甜品菜单、用户中心（充值/抽奖/优惠券/评价）一应俱全。
...
管理员账号：`admin`。**初始密码为首次启动时随机生成**（见下方「安全说明」），首次登录必须修改密码后才能使用后台。
...
- 用户管理：查看注册用户，管理员可调整任意用户余额（带备注，自动记账）
- 评价管理：审核用户评价（通过后展示在首页），可驳回或删除
- 修改密码：后台右上角「修改密码」可随时更换管理员密码（至少 8 位）

## 用户中心（用户端功能）

前台导航栏「用户中心」入口，注册 / 登录后可使用：

- **余额充值**：模拟充值（无真实支付，后续可接入微信/支付宝），充值记录自动记入余额明细
- **幸运抽奖**：每天 3 次免费机会，随机抽取 ¥5 / ¥10 无门槛券或满减券（有效期 30 天）
- **我的优惠券**：查看抽中的优惠券
...
- **登录防爆破**：同一账号+IP 连续失败 5 次，锁定 15 分钟
